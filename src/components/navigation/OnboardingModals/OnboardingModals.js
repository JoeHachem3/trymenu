...
export const Cusine = (props) => {
    return (
        <a onClick={() => props.onPress ? props.onPress() : {}}>
            <div className={props.selected ? props.dark ? classes.cusineContainerSelectedDark : classes.cusineContainerSelected : props.dark ? classes.cusineContainerDark : classes.cusineContainer}>
...
const FavCusinesScene = (props) => {
    ...
    return (
        <div className={classes.onboardingContainer}>
            <span className={classes.onboardingContainerTitle}>Hello,  {user?.username}!</span>
            <span className={classes.onboardingContainerSub}> In order to make better reommendations, select your favortie cuisines</span>
            <div className={classes.onboardingContainerContent}>
                {cusines.map((cuisine, idx) => (
                    <Cusine selected={(selectedCuisine.includes(idx))} label={cuisine} onPress={() => {
    ...
    return (
        <>
            {showModal ? (
                ...
                >
                    <Container onClose={() => setShowModal(false)} onNext={() => setOnboard(x => x+1)}>
                        {renderOnboard()}
                    </Container>
                </Modal>
            ) : null}
