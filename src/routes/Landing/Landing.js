...
class Landing extends Component {
    ...
    inputChangedHandler = (event, inputIdentifier, form) => {
        ...
        updatedFormElement.valid = checkInputValidity(updatedFormElement.value, updatedFormElement.validation)
        ...
        this.setState({[form]: { form: updatedOrderForm, formIsValid: formIsValid }})
    }
            ...
            form = (
                <form onSubmit={this.submitHandler}>
                    ...
                    return(<Input 
                        ...
                        elementConfig={{...formElement.config.elementConfig, placeholder: (formElement.config.elementConfig.placeholder)}} 
                        ...
                        hideLabel
                        changed={(event) => this.inputChangedHandler(event, formElement.id, 'login')}/> 
                    )})}
                    <Button disabled={!this.state.login.formIsValid}>{("Sign in")}</Button>
                    <div className={classes.LoginToggler}>{this.state.isLogin ? 'No account? ' : 'Have an account? '}<span onClick={this.toggleLogin}>{this.state.isLogin ? 'Signup' : 'Login'}</span></div>
                </form>
            );
        } else {
            ...
            form = (
                <form onSubmit={this.submitHandler}>
                    ...
                    return(<Input 
                        ...
                        hide={formElement.config.hide}
                        hideLabel
                        changed={(event) => this.inputChangedHandler(event, formElement.id, 'register')}/> 
                    )})}
                    <Button disabled={!this.state.register.formIsValid}>{("Register")}</Button>
                    <div className={classes.LoginToggler}>{this.state.isLogin ? 'No account? ' : 'Have an account? '}<span onClick={this.toggleLogin}>{this.state.isLogin ? 'Signup' : 'Login'}</span></div>
                </form>
            );
        }
